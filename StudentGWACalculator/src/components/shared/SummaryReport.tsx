...
export const SummaryReport: React.FC<SummaryReportProps> = ({
    ...
    // Styles (always light mode for "Professional Report" look, or a nice dark card)
    // Let's go with a specialized clean look that works well when exported
    // We will force a light theme for the report so it prints nicely on paper too
    const bg = 'bg-white';
    const text = 'text-gray-900';


    // Check for violations in the items provided to this report (Current record)
    ...
    // If global violation info is provided (from history), use it. 
    // Otherwise, assume term violation is the only known violation.
    ...
    // Check if there is at least one subject with a final grade
